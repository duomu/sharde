 ...
 */
public class DefaultTableShardingAlgorithm implements SingleKeyTableShardingAlgorithm<Integer> {
    ...
    public Collection<String> doInSharding(Collection<String> tableNames, ShardingValue<Integer> shardingValue) {
        ...
        for (Integer value : shardingValue.getValues()) {
            for (String each : tableNames) {
                if (each.endsWith((value / TABLE_USER_AMOUNT + 1) + "")) {
                    result.add(each);
                }
            }
        }
    ...
    public Collection<String> doBetweenSharding(Collection<String> tableNames, ShardingValue<Integer> shardingValue) {
        ...
        for (Integer i = range.lowerEndpoint(); i <= range.upperEndpoint(); i++) {
            for (String each : tableNames) {
                if (each.endsWith((i / TABLE_USER_AMOUNT + 1) + "")) {
                    result.add(each);
                }
            }
        }
